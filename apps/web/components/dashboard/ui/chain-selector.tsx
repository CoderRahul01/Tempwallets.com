...
export function ChainSelector({
  ...
  const allChains = useMemo(() => {
    ...
    return walletConfig.all
      .filter((config) => {
        // Feature flags: enabled check
        const isEnabled = isDev ? config.features.enabledInDev : config.features.enabledInProd;
        if (!isEnabled) return false;

        // Only show testnets if we are in dev or if they are explicitly marked as showInSelector
        if (config.isTestnet && !isDev && !config.features.showInSelector) {
          return false;
        }
      ...
      <div className={cn('rounded-3xl p-3 md:p-4 mt-4', className)} style={{ backgroundColor: '#292828' }}>
        ...
        <div className="overflow-x-auto overflow-y-hidden scrollbar-hide">
          <div className="flex gap-3 snap-x snap-mandatory">
            ...
            {/* Placeholder removed to show all available networks */}
          </div>
        </div>
      </div>
